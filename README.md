...
This project takes a real, messy two year retail transaction dataset and treats it the way an actual retail analytics function would, cleaning it, structuring it properly, and using it to answer specific business questions rather than working through techniques for their own sake. Which products actually drive revenue, which customers are worth focusing retention effort on, and how much stock a product genuinely needs, all sit behind the work here, cleaning and profiling raw data in Power Query, designing and building a normalized database in PostgreSQL, writing analytical SQL against it, and finishing with a two page dashboard connected directly to that database.

This was self directed, not client work, but built with the same discipline a real analytics project would need, decisions documented, numbers checked before being trusted, and limitations stated plainly rather than smoothed over. The full reasoning behind every decision sits in the accompanying report. This README pulls out what's worth knowing at a glance, what the data looked like going in, what needed fixing and why, how the database is structured, and what the analysis actually found.
...
Each of these came with a documented decision, not just a fix, and the full reasoning behind each one is in the report. The sixteen non product codes in particular mattered beyond tidiness, leaving postage charges or manual adjustments inside a revenue figure would have overstated real sales performance, the kind of judgement call that sits behind any retail reporting someone is actually meant to trust.
...
The structure itself was shaped around how a business would actually need to query it, customers, products and transactions held separately rather than one flat file, so a question about revenue, product performance or customer behaviour could each be answered directly instead of reconstructed from scratch every time.

## The SQL analysis

With the database built, the analysis worked through the kind of questions a merchandising, retention or planning function would actually ask:
...
- A full RFM customer segmentation, recency, frequency, and monetary value, scored into quartiles and grouped into four segments, turning "focus on our best customers" into an actual, usable list rather than a vague intention
...
- Average monthly demand per product, checked closely enough to catch a product whose apparent demand was driven almost entirely by a single bulk order rather than steady sales, a distinction that matters directly for purchasing decisions and directly shaped how inventory_snapshot was built
...
This was built as if a retail analytics or planning team needed it, not because one actually did. No business acted on these findings, and none of the numbers here reflect a real company's performance. The value of the project sits in the decisions and the reasoning behind them, not in a claim of business impact that never happened.
